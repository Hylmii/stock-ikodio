...
export default function HomePage() {
  ...
  const handleAuthSuccess = (email: string, isNewUser: boolean) => {
    console.log("Auth success:", { email, isNewUser });
    setUserEmail(email);
    setIsAuthenticated(true);

    // Save auth to localStorage immediately
    localStorage.setItem("ikodio_auth", "true");
    localStorage.setItem("ikodio_email", email);
    localStorage.setItem("ikodio_kyc_completed", "true"); // Set KYC as completed

    // NOTE: Don't manually set session cookie - Better Auth handles this automatically
    console.log("Session managed by Better Auth");

    // Redirect all users to dashboard (skip KYC for now)
  ...
  const handleKYCComplete = () => {
    ...
    if (userEmail) {
      localStorage.setItem("ikodio_email", userEmail);
      // NOTE: Don't manually set session cookie - Better Auth handles this
      console.log("Session managed by Better Auth");
    }
