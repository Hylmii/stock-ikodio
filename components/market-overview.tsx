...
export function MarketOverview() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Stat values - Subtle pop
      const statValues = statsGridRef.current?.querySelectorAll(".stat-value");
      if (statValues) {
      ...
      // Hover effects for stat cards - Minimal
      if (statsGridRef.current?.children) {
        Array.from(statsGridRef.current.children).forEach(card => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative bg-[#0A0A0A] border-t border-white/5 py-16"
    >
        ...
        >
          {stats.map((stat, index) => (
            ...
            >
              ...
              {/* Value */}
              <div className="stat-value text-3xl font-semibold text-white mb-1">
                {stat.value}
              </div>
